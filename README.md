...
- **Architecture Highlights:** 
  - Single Page Application (SPA) with lazy-loaded modules for optimal performance.
  - Standalone Components and Reactive Forms for streamlined development.
  - Centralized state management and HTTP interceptors for authentication, error handling, and API communication.
  - Fully responsive and mobile-friendly UI.

### ⚙️ Backend (Server-Side)
- **Framework:** ASP.NET Core 10.0 Web API (C#)
- **Architecture:** N-Tier Architecture utilizing the **Repository Pattern** for separation of concerns, testability, and clean code.
- **Key Features:**
  - RESTful API design with structured JSON responses.
  - Dependency Injection (DI) for centralized service registration.
  - JWT-based Authentication and Role-Based Access Control (RBAC).
  - Complex business logic handling (e.g., dynamic loan amortization, strict inventory tracking).

### 🗄️ Database
- **Engine:** Azure SQL Server
- **ORM:** Entity Framework Core (EF Core) using Code-First approach.
- **Structure:** Fully normalized relational database handling multi-entity relationships like User Roles, Stock Entries, Loan EMIs, Tax Rules, and more.
